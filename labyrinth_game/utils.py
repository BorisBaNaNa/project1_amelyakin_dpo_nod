...
def solve_puzzle(game_state):
    ...
    # для расширяемости список верных ответов записывается прям в константах
    # чтобы не бегать и не искать где алитернативные ответы
    question, correct_answers = puzzle
    print(question)
    ...
    if user_answer in [ans.lower() for ans in correct_answers]:
        ...
        room_rewards = {
            ...
            "treasure_room": ("gold coin", "За решение загадки вы получили монету!"),
        }

        default_reward = ("gold coin", "За решение загадки вы получили монету!")
        reward_item, reward_message = room_rewards.get(current_room_key, default_reward)
